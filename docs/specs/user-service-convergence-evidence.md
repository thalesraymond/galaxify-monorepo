...
committed serial Playwright run: **13 passed** (25.8 s), incl. the outage test
which stops and restarts the real service:

```text
✓ 1 wire contract / health / signup / me / patch / delete        (668ms)
✓ 2 refresh single-use: consume once, reuse revokes the family   (1.1s)
✓ 3 logout: refresh_token body, no auth header, 204, revoked     (591ms)
✓ 4 login: normalized email                                      (618ms)
✓ 5 signup + bootstrap rotation on reload                        (2.4s)
✓ 6 threshold gating: fresh token, no premature rotation         (1.6s)
✓ 7 reactive refresh: one 401 -> one real rotation -> one replay (1.6s)
✓ 8 cross-tab serialization: 2 concurrent reloads, zero 401s     (3.4s)
✓ 9 terminal invalidity syncs every tab                          (3.3s)
✓10 profile: get / update / validation / real 409 conflict       (2.6s)
✓11 logout (UI): family revoked, state cleared                   (2.2s)
✓12 deletion: password-confirmed, no logout call                 (1.6s)
✓13 retryable outage: kill -> unavailable -> restart -> Retry    (2.7s)
```
...
| 5 | Signup + bootstrap rotation | Signup enters the Dashboard; only the versioned refresh-token key is persisted (no access token); reload restores the session through a real startup rotation (stored token changes; consumed startup token is single-use 401). |
...
| 7 | Reactive refresh | One fabricated 401 `AUTH_INVALID_TOKEN` at the browser network boundary → exactly one real `POST /auth/refresh` rotation → exactly one replay of `GET /users/me` (200) → session recovered. |
...
| 10 | Profile get/update | Read-only email + editable username from `GET /users/me`; inline client validation; real 409 → inline "That username is already taken."; update persists across reload via `PATCH /users/me`. |
...
The outage test finds the listener with `lsof -ti tcp:8081 -sTCP:LISTEN`, sends
`SIGTERM`, waits until the proxy health check fails, reloads the app, restarts
`./bin/user-service` from `apps/user-service`, and clicks Retry.
